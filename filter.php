	...
	if ($rowCount > 0) {
		...
		while ($row = $qry_result->fetch_assoc()) {
	       ...
	       $display_string .=  '<a href="product.php?id=' . $product->getid() . '"><img src="inventory_images/'
	        . $product->getid() . '.png" alt="' . $product->getname() . '" width="200" height="100" border="1" /><br></a>'. $product->getname() . '<br /> $' . $product->getprice() . '<br /> <a onclick = "product()" href="productNew.php?id=' . $product->getid() . '"><button name = "View Product Details" type = "button" id = "productId" value = '.$product->getid().'></button></a><br /> </div>';
	    }
	   } else {
