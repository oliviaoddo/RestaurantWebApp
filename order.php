<?php
	...
	//get JSON object that was sent from completeOrder.js
	//create the object $data with its contents
	...
	//store customer info in variables
	$fname = $data->info[0];
	$lname = $data->info[1];
	$phone = $data->info[3];
	$email = $data->info[2];
	...
	$selectUserID = "SELECT user_id FROM USERS WHERE fname ='$fname' and lname = '$lname' LIMIT 1";
	...
	if (isset($_SESSION["customer"])) {
		...
		$userSQL = "INSERT INTO users VALUES(default,'$fname', '$lname', '$phone','$email')";
		...
		$subclassSQL = "INSERT INTO GUEST VALUES(default,'$userID')";
	...
	/*CHECKOUT STEP 2, inserting order/orderline/address(es) into tables*/
	/* credit card input */
	$orderType = $data->info[23];
	$order_time = null;
	if ($orderType == "pickup"){
